  ...
  <div ref="searchGroup" class="search_group">
    ...
    >
      <el-row class="search_form_row">
        <el-col :span="5">
          <el-form-item label="客户姓名" prop="username" class="search-form-item">
            <el-input
        ...
        <el-col :span="5">
          <el-form-item label="客户昵称" prop="nickname" class="search-form-item">
            <el-input
...
<script lang="ts" setup name="baseSearch">
  ...
  import { CustomerQueryParams } from '/@/views/upms/model/customerModel';
  const emit = defineEmits(['isFolder', 'queryTable']);
  const props = defineProps(['queryData']);
  const queryFormRef = ref<FormInstance>();
  const queryForm = ref<CustomerQueryParams>({
    ...props.queryData,
  });
  /**
   * 有必输校验就加，没有就删除
