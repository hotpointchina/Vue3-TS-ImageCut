    ...
    <div class="flex flex-col w-full px-20px">
      ...
      <div class="base-table">
        ...
        >
          ...
          <el-table-column property="activitiesCode" label="活动编号" width="180" />
          <el-table-column
          ...
          <el-table-column
            ...
            label="公益组织名称"
          />
          <!--          <el-table-column property="explainn" :show-overflow-tooltip="true" label="活动简介" width="200" />-->
          <!--          <el-table-column property="donationFunded" :show-overflow-tooltip="true" label="已筹善款" width="150" />-->
          <!--          <el-table-column property="donationNumber" :show-overflow-tooltip="true" label="捐赠人数" width="150" />-->
          <el-table-column
          ...
          <el-table-column
            ...
            label="申请时间"
          />
          <!--          <el-table-column property="activitiesEndTime" :show-overflow-tooltip="true" label="截至日期"  />-->
          <!--          <el-table-column property="activitiesType" :show-overflow-tooltip="true" label="捐赠类型"  >-->
          <!--            <template #default="scope">-->
          <!--              <span v-if="scope.row.activitiesType === '0'">公益</span>-->
          <!--              <span v-if="scope.row.activitiesType === '1'">个人</span>-->
          <!--            </template>-->
          <!--          </el-table-column>-->
          <el-table-column
            property="status"
          ...
          <el-table-column fixed="right" label="操作" width="280">
            <template #default="scope">
              ...
              <el-button
                ...
                v-if="
                  scope.row.flagStatus === '待审核' || scope.row.flagStatus === '已驳回'
                "
                >编辑</el-button
              >
              <el-button
                ...
                v-if="
                  scope.row.flagStatus === '待提交' || scope.row.flagStatus === '已驳回'
                "
                >提交</el-button
              >
              <el-button
                ...
                >{{
                  scope.row.status === '停用' || scope.row.status === '停用'
                    ? '启用'
                    : '停用'
                }}</el-button
              >
              ...
              <el-button
                ...
                v-if="
                  scope.row.flagStatus === '已发布' || scope.row.flagStatus === '已结束'
                "
                >捐赠情况</el-button
              >
              <el-button
      ...
      <!-- 进度维护 -->
      <div v-if="isShowAddProduct">
        <el-dialog
          v-model="isShowAddProduct"
          width="50%"
          top="15vh"
          :draggable="true"
          title="进度维护"
...
<script lang="ts" setup name="crowdfundingactivities">
  ...
  import editPage from './edit/editCrowd.vue';
  ...
  import { doneeDownload, getBase64 } from '/@/views/upms/api/doneeDecla';
  ...
  const isShowAddProduct = ref<boolean>(false); // 进度维护
  // 函数的类型
 ...
 */
  ...
  const deleteChoose = () => {
    if (multipleSelection.value!.length < 1) {
      ...
      multipleSelection.value.forEach((item) => {
        if (item.flagStatus === '待提交' || item.flagStatus === '已驳回') {
  ...
  const detailClick = (row: CrowdListParams) => {
    ...
    getOnlyActivitys(row.id).then((res) => {
      ...
      Data = res.activitiesMrg;
      console.log('res.activitiesMrg: : ', res.activitiesMrg);
      doneeDownload(res.activitiesMrg).then((res) => {
        getBase64(res.data, res.data.type).then((base64) => {
          let value = base64 as string;
          const idCardMsgs = `<img style="height:130px;" src="${value}"/>`;
          console.log('idCardMsgs: ', res);
          return activitiesMrg.value.push(idCardMsgs);
          // return (detailInfo.value.idCardMsg = value);
        });
      });

      res.spreadImage.forEach((key) => {
      ...
      // doneeData = res.activitiesMrgList;
  ...
  //进度维护
  const addSchedule = (row: detailMany) => {
  ...
  const statusHandler: handlerFunction<CrowdListParams> = (row: CrowdListParams) => {
    ...
    if (row.status === '启用') {
      ElMessageBox.confirm('确认禁用已选中的数据项?', '警告', {
        ...
        .then(() => {
          editStatus(row.id, { status: '停用' }).then(() => {
            getTableList();
          });
        })
        .catch(() => ElMessage.info('已取消'))
