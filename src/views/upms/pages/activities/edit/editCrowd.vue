<template>
  <div class="dept_all">
    ...
    <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="200px"
      ...
      <!--  activitiesMrg    spreadImage-->
      <el-form-item label="活动图片" prop="spreadImage" class="w-590px mr-40px">
        <el-upload
          ref="uploadRef"
      ...
      <el-form-item label="有效证明" prop="activitiesMrg" class="w-590px mr-40px">
        <el-upload ref="uploadRef"
                   ...
                   name="file"
                   :on-change="handleChange" :limit="6"
...
<!--      <el-form-item class="w-430px mr-40px" />-->
      <el-form-item label="项目名称" prop="activitiesName" class="w-590px mr-40px">
        <el-input :autofocus="true" v-model="ruleForm.activitiesName" placeholder="请输入项目名称" ref="cmd"
                  :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="活动备案编号" prop="activitiesRecordCode" class="w-590px mr-40px">
        <el-input v-model="ruleForm.activitiesRecordCode" placeholder="请输入活动备案编号" :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="活动备案编号有效期" prop="activitiesRecordTime" class="w-590px mr-40px">
<!--        <el-input v-model="ruleForm.activitiesRecordTime" placeholder="请输入活动备案编号" :disabled="crowdStore.justView" />-->
        <el-date-picker v-model="ruleForm.activitiesRecordTime" type="datetime" format="YYYY-MM-DD HH:mm:ss"
                        value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择活" :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="活动负责人" prop="head" class="w-590px mr-40px">
        <el-input v-model="ruleForm.head" placeholder="请输入负责人" :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="联系方式" prop="phoneNumber" class="w-590px mr-40px">
        <el-input v-model="ruleForm.phoneNumber" placeholder="请输入联系方式" :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="负责人简介" class="w-590px mr-40px">
        <el-input v-model="ruleForm.headIntroduction" placeholder="请输入内容" :disabled="crowdStore.justView" />
      </el-form-item>
      <!--      <el-form-item label="活动期限" prop="activitiesTerm" class="w-590px mr-40px">-->
      <!--        <el-radio-group v-model="ruleForm.activitiesTerm" :disabled="crowdStore.justView">-->
      <!--          <el-radio label="长期" />-->
      <!--          <el-radio label="短期" />-->
      <!--        </el-radio-group>-->
      <!--      </el-form-item>-->
      <el-form-item label="筹款目标" prop="donationTarget" class="w-590px mr-40px" >
        <el-input v-model="ruleForm.donationTarget" placeholder="请输入" :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="活动开始时间" prop="activitiesStartTime" class="w-590px mr-40px">
        <el-date-picker v-model="ruleForm.activitiesStartTime" type="datetime" format="YYYY-MM-DD HH:mm:ss"
                        value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择活动开始时间" :disabled="crowdStore.justView" />
      </el-form-item>
      <el-form-item label="活动结束时间" prop="activitiesEndTime" class="w-590px mr-40px">
        <el-date-picker v-model="ruleForm.activitiesEndTime" type="datetime" format="YYYY-MM-DD HH:mm:ss"
                        value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择活动结束时间" :disabled="crowdStore.justView" />
      </el-form-item>
      ...
      <el-form-item label="活动简介" prop="explainn" class="w-1222px mr-40px">
        <el-input v-model="ruleForm.explainn" placeholder="请输入内容" type="textarea" :disabled="crowdStore.justView" />
      </el-form-item>
    </el-form>
    <!-- 富文本 -->
    <el-form v-for="(item, index) in quillFormFor" :ref="'quillFormRef' + index" :model="quillForm" :rules="quillRules"
             label-width="84px" class="gy-form flex flex-col flex-wrap justify-start item-start w-full" size="default"
             status-icon>
      <el-row class="w-full">
        <el-form-item :label="item.label" :prop="item.value" :key="item.value" class="w-1230px mr-40px h-300px"
                      @click="quillFocus(index)">
          <QuillEditor v-if="!crowdStore.justView" :content="quillForm[item.value]" :imgageUrl="item.imgageUrl"
                       :isUpdated="item.isUpdated" @change="(obj) => btnHandler(obj, index)" @blur="(obj) => btnHandler(obj, index)"
                       @uploadFile="(obj) => quillUploadFile(obj)" :isError="item.isError" />
          <div class="justsee_box" v-if="crowdStore.justView" v-html="quillForm[item.value]"></div>
        </el-form-item>
      </el-row>
    </el-form>
    <el-row class="flex justify-center w-full mt-30px" v-if="!crowdStore.justView">
      ...
      <el-button type="primary" @click="submitFrom(ruleFormRef)" v-if="crowdStore.pageTitle === '众筹活动申报新增'">保存</el-button>
      <!--      <el-button type="primary" @click="addSubmitFrom(ruleFormRef)" v-if="-->
      <!--        crowdStore.pageTitle === '众筹活动申报新增' ||-->
      <!--        crowdStore.pageTitle === '众筹活动流程申请'-->
      <!--      ">提交</el-button>-->

      <el-button type="primary" @click="submitFrom(ruleFormRef)" v-if="crowdStore.pageTitle == '众筹活动申报编辑'">修改</el-button>
    </el-row>
  </div>
</template>
...
const handlesuggestMsgRemove = (uploadFile, uploadFiles: string[]) => {
  ...
  console.log(uploadFile, uploadFiles, suggestMsgList,'  :suggestMsgList');
};
const handlesuggestMsgSuccess: UploadProps['onSuccess'] = (
  ...
  console.log(suggestMsgList.value,' 11111111111111---suggestMsgList')
...
const resetForm = () => {
  ...
  Object.keys(quillFormFor).forEach((key) =>{
    if (key !== 'pageNum' && key !== 'pageSize') {
      quillFormFor[key] = '';
    }
  });
  ...
  // ruleForm.endTime = endDate;
  // ruleForm.startTime = startDate;
};
// 活动结束时间验证
...
// 新增/编辑 表单
// 新增/编辑 表单
// 新增、添加名址表单的校验规则  spreadImage   activitiesMrg
const rules = reactive<FormRules>({
  spreadImage: [{ required: true, message: '请上传图片', trigger: 'change' }],
  activitiesMrg: [{ required: true, message: '请上传图片', trigger: 'change' }],
  activitiesName: [
    {
      required: true,
  ...
  surplusPropertyDispose: [
    { required: true, message: '剩余财产处置', trigger: 'blur' },
  ],
});
//富文本数组
...
const btnHandler = (obj: any, index: string | number) => {
  switch (index) {
    case 0:
      ...
      console.log('introduce11111-: ',obj.content)
    ...
    case 3:
      ...
      break;
    case 4:
      quillForm.surplusPropertyDispose = obj.content == '<p><br></p>' ? '' : obj.content;
  ...
  switch (hangle.value) {
    case 0:
      uploadFile(obj.formData).then((res) => {
        ...
        quillFormFor[0].imgageUrl = res.data.result;
    ...
    case 1:
      uploadFile(obj.formData).then((res) => {
        quillFormFor[1].imgageUrl = res.data.result;
      });
      break;
    case 2:
      uploadFile(obj.formData).then((res) => {
        quillFormFor[2].imgageUrl = res.data.result;
      });
      break;
    case 3:
      uploadFile(obj.formData).then((res) => {
        quillFormFor[3].imgageUrl = res.data.result;
      });
      break;
    case 4:
      uploadFile(obj.formData).then((res) => {
        quillFormFor[4].imgageUrl = res.data.result;
      });
      break;
  }
  loading.close();
...
const handleAvatarSuccess: UploadProps['onSuccess'] = (
  ...
  spreadImageList.value.push( response.result.filePath );
...
// 下载图片;
const handleDownload = (spreadImageList: string[]) => {
  // console.log('1111111111');
  // fileList.value = spreadImageList.value;
  // spreadImageList.forEach((it) => {
...
const download = (suggestMsgList: string[]) => {
  // console.log('1111111111');
  // fileList.value = suggestMsgList.value;
  // spreadImageList.forEach((it) => {
  //   fileList.value.push({ url: it });
  // });
  suggestMsgList.forEach((it) => {
    // let blob = new Blob([item.data]); // 返回的文件流数据
    // let url = window.URL.createObjectURL(blob); // 将他转化为路
    // imageUrl.value = url;
    // fileList.value.push({ url: value, id: it });
...
const submitFrom = async (formEl: FormInstance | undefined) => {
  ...
  let imgList = '';
  // 上传图片的id
  for (let i = 0; i < spreadImageList.value.length; i++) {
    if (i == 0) {
      imgList = spreadImageList.value[i];
    } else {
      imgList = imgList + ',' + spreadImageList.value[i];
    }
  }
  let imgLists = '';
  for (let i = 0; i < suggestMsgList.value.length; i++) {
    if (i == 0) {
      imgLists = suggestMsgList.value[i] as string;
    } else {
      imgLists = imgList + ',' + suggestMsgList.value[i] as string;
    }
  }
  ruleForm.spreadImage = imgLists;
...
const addSubmitFrom = async (formEl: FormInstance | undefined) => {
  ...
  for (let i = 0; i < suggestMsgList.value.length; i++) {
    if (i == 0) {
      imgLists = suggestMsgList.value[i];
    } else {
      imgLists = imgLists + ',' + suggestMsgList.value[i];
    }
  }
  ruleForm.activitiesMrg = imgLists;
  ruleForm.spreadImage = imgList;
  ...
  if (crowdStore.pageTitle == '众筹活动申报编辑') {
    ...
    getOnlyActivitys(crowdStore.$state.editData.id)
      .then((res) => {
        ...
        spreadImageList.value = res.activitiesMrgList;

        if (spreadImageList.value.length > 0) {
          handleDownload(spreadImageList.value);
        }
        ...
        console.log(quillForm,'quillForm')
        setTimeout(() => {
    ...
    if (props.flowData && props.flowData.editData.projectId !== '') {
      try {
        getOnlyActivitys(props.flowData.editData.projectId).then((res) => {
          ...
          spreadImageList.value = res.spreadImageList;
          if (spreadImageList.value.length > 0) {
            handleDownload(spreadImageList.value);
          }
          suggestMsgList.value = res.spreadImage;
          console.log('res.spreadImageList111222333 :  ',res.spreadImage)
          if (suggestMsgList.value.length > 0) {
            download(suggestMsgList.value);
          }
          ...
          console.log(quillForm,'quillFormFor')
          setTimeout(() => {
            quillFormFor.forEach((key) => {
              key.isUpdated = false;
            });
