  ...
  <div class="dept_all">
    ...
    >
      <el-tab-pane name="first">
        <template #label>
          未读
          <el-badge v-if="total != 0" :value="total" class="item"> </el-badge>
        </template>
              ...
              <el-table-column fixed="right" label="操作" width="120">
                <template #default="scope">
                  <el-button
                    ...
                    type="primary"
                    size="small"
                    @click="detailReadClick(scope.row)"
                    >详情</el-button
                  >
                </template>
              </el-table-column>
...
<script lang="ts" name="notice">
  ...
  import {
    ...
    messageReadDel,
    messageReadID,
  } from '/@/views/upms/api/messageCenter';
  ...
  export default {
    ...
    setup() {
      ...
      const tableDataRead = ref<NoticeListParams[]>([]);
      ...
      const detailReadClick = (row: NoticeListParams) => {
        // 直接获取表格中的数据，或者访问后台代码
        detailInfo.value = row;
        detailShow.value = true;
      };
      ...
      function getTableList(params: NoticeQueryParams) {
        ...
        messageReadList(params).then((res) => {
          tableDataRead.value = res.records;
          totalRead.value = res.total;
        });
      }
