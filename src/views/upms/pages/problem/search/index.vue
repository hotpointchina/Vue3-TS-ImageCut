  ...
  <div ref="searchGroup" class="search_group">
    ...
    >
      <el-row class="search_form_row">
        <el-col :span="5">
          <el-form-item label="问题标题" prop="questionHead" class="search-form-item">
            <el-input
              v-model="queryForm.questionHead"
              clearable
              placeholder="请输入问题标题"
            />
          </el-form-item>
        ...
        <el-col :span="5">
          <el-form-item label="问题类型" prop="questionType" class="search-form-item">
            <el-select
              v-model="queryForm.questionType"
              clearable
              placeholder="问题类型"
            >
              <el-option label="账号问题" value="0" />
              <el-option label="举报" value="1" />
              <el-option label="物流问题" value="2" />
              <el-option label="订单问题" value="3" />
              <el-option label="奖励规则" value="4" />
              <el-option label="支付问题" value="5" />
              <el-option label="其他问题" value="6" />
            </el-select>
          </el-form-item>
        </el-col>
...
<script lang="ts" setup name="baseSearch">
  ...
  import { ProblemQueryParams } from '/@/views/upms/model/problem';
  const emit = defineEmits(['isFolder', 'queryTable']);
  const props = defineProps(['queryData']);
  const queryFormRef = ref<FormInstance>();
  const queryForm = ref<ProblemQueryParams>({
    ...props.queryData,
  });
  /**
   * 有必输校验就加，没有就删除
