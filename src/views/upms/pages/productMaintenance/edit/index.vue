  ...
  <div class="dept_all" element-loading-text="Loading...">
    ...
    <div style="margin-left: 250px;margin-top: 50px;">
      <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="120px"
        ...
        <br>
        <el-row class="flex justify-center w-full mr-450px">
          <el-form-item>
...
import { editData, getById } from '/@/views/upms/api/productMaintenance';
...
const ruleForm = reactive<tableDataI>({
  wareName: '',
  wareBrand: '',
  wareType: '',
  suitAge: '',
...
onMounted(() => {
  getById(props.MSG).then((res) => {
    ruleForm.id = res.id
    ruleForm.wareName = res.wareName
    ruleForm.wareBrand = res.wareBrand
    ruleForm.wareType = res.wareType
    ruleForm.suitAge = res.suitAge
    ruleForm.wareIntroduce = res.wareIntroduce
    ruleForm.wareMsg = res.wareMsg[0]
    fileListreve.value[0].name = res.wareName
    fileListreve.value[0].url = res.wareMsg[0]
  })
})
/** 图片*/
...
const handleReveSuccess: UploadProps['onSuccess'] = (
  ...
  ruleForm.wareMsg = response.result.fileUrl
