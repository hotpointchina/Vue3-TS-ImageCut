...
<template>
  <div class="dept_all">
    <div class="search_top flex flex-row justify-between items-center w-full pb-9px">
      <h1 class="page_title">受捐人申报{{ buttonType == '1' ? '新增' : '编辑' }}</h1>
    </div>
...
<script lang="ts" setup name="addressAdd">
    ...
    if (useUpmsAddressStore.$state.isEditing) {
      ...
      doneeDetail(useUpmsAddressStore.$state.editDataInfo.id as string)
        .then((res) => {
          ...
          handleDownloadLi();
          handleDownloadreve();
          suggestMsgList.value = res.suggestMsg;
  ...
  const rules = reactive<FormRules>({
    ...
    doneeEndTime: [{ required: true, message: '请选择筹款时间', trigger: 'change' }],
  });
  ...
  //推荐图片 下载图片;
  // const handlEsuggestMsgDownload = () => {
  //   // suggestMsgList.value = (ruleForm.value.suggestMsg as string).split(',');
  //   suggestMsgList.value = ruleForm.value.suggestMsg as string[];

  //   let Data = '';

  //   suggestMsgList.value.forEach((key) => {
  //     Data = key;
  //     doneeDownload(Data).then((res) => {
  //       getBase64(res.data, res.data.type).then((base64) => {
  //         let value = base64 as string;
  ...
  const submitFrom = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate((valid) => {
      if (
        activitiesMrgList.value.length > 0 &&
        ReveList.value.length > 0 &&
        suggestMsgList.value.length > 0 &&
        doneeMsgMrgList.value.length > 0
      ) {
        // let imgList = '';
        let doneeimgList = '';
        let suggestList = '';
        ...
        // else {
        //   // imgList = imgList + ',' + activitiesMrgList.value[i].url;
        //   reveImgList = reveImgList + ',' + ReveList.value[i];
        // }
        // }
        ...
        for (let i = 0; i < suggestMsgList.value.length; i++) {
          if (i == 0) {
            suggestList = suggestMsgList.value[i];
          } else {
            suggestList = suggestList + ',' + suggestMsgList.value[i];
          }
        }
        ruleForm.value.idCardMsg = activitiesMrgList.value[0] + ',' + ReveList.value[0];

        // ruleForm.value.idCardMsg = imgsMsg.value.join();
        ruleForm.value.frontID = activitiesMrgList.value[0];
        ruleForm.value.reverseID = ReveList.value[0];
        ruleForm.value.doneeMsg = doneeimgList;
        ruleForm.value.suggestMsg = suggestList;
      } else {
        return ElMessage({
          message: '请上传图片',
          type: 'error',
      ...
      if (
        ruleForm.value.doneeName == ruleForm.value.urgentName ||
        ruleForm.value.phoneNumber == ruleForm.value.urgentPhone
      ) {
        ElMessage({
          message: '受捐人姓名跟电话不可以跟紧急联系人一致',
          type: 'error',
        });
        return;
      }
